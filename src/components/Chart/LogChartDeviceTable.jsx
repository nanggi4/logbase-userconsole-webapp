...
//=========================================================================
const LogChartDeviceTable = React.memo((props) => {
  ...
  return useObserver(() => (
    <React.Fragment>
      <Paper elevation={3} className={props.classes.innerWrap}>
        <h5 className={props.classes.title}>디바이스 접속현황</h5>
        <Table className={props.classes.dataTable} aria-label="hourlyCount data table">
          <TableBody>
            {chartStore.deviceCount.map((datas, idx) => (
              datas.map(data => {
                const device = data['device'] !== null ? `${data['device']}` : `${data['device']}`;
                const count = data['count'] !== null ? `${data['count']}건` : `0건`;
                return (
                  <TableRow className={props.classes.tableTR} key={idx}>
                    <TableCell className={props.classes.tableTD} align={'center'}>{device}</TableCell>
...
//=========================================================================
export default withStyles(styles,{withTheme:true})(LogChartDeviceTable);