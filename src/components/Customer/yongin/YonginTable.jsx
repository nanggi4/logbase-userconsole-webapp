...
//=========================================================================
const yonginTable = (props) => {
  const { logStore, yonginStore } = useStore();
  ...
  return useObserver(() => (
    <React.Fragment>
      <Button onClick={handleTableOption()} variant="contained" size="small" color="primary" style={{ marginTop: 8, marginBottom: '0.25rem' }}>table options</Button>
      {yonginStore.yonginSourceList.length>0 && (
        <Table className={props.classes.dataTable} style={{ maxWidth: 300, margin: '0 0 0.25rem' }}>
          <TableHead className={props.classes.dataTableHead}>
            <TableRow>
              <TableCell className={clsx(props.classes.tableTH)} >매체</TableCell>                
              <TableCell className={clsx(props.classes.tableTH)} >건수</TableCell>                
            </TableRow>
          </TableHead>      
          <TableBody className={props.classes.dataTableBody}>
            {yonginStore.yonginSourceList.map((row, index) => {
              let name = row[0] === '홈페이지 배너' ? '자연유입' : row[0];
              return (
                <TableRow className={clsx(props.classes.tableTR)} key={index}>
                  <TableCell className={clsx(props.classes.tableTD)} align={'center'}>{name}</TableCell>
                  <TableCell className={clsx(props.classes.tableTD)} align={'center'}>{row[1]}</TableCell>
                </TableRow>            
              );
            })}                
          </TableBody>
      ...
      {yonginStore.yonginReward.length>0 && (
        <React.Fragment>
          <h3 className={props.classes.subTitle}>채결 고객</h3>
          <Table stickyHeader aria-label='sticky table' size='small' className={props.classes.dataTable} style={{ marginTop: '0.25rem' }}>
            <colgroup>
              <col width='80px' />
              <col width='170px' />
              <col width='130px' />
            ...
            <TableBody className={props.classes.dataTableBody}>
            {(yonginStore.yonginReward.map((row) => (
      ...
      {yonginStore.yonginPoten.length>0 && (
        <React.Fragment>
          <h3 className={props.classes.subTitle}>잠재 고객</h3>
          <Table stickyHeader aria-label='sticky table' size='small' className={props.classes.dataTable} style={{ marginTop: '0.25rem' }}>
            <colgroup>
              <col width='80px' />
              <col width='170px' />
            ...
            <TableBody className={props.classes.dataTableBody}>
            {(yonginStore.yonginPoten.map((row) => (
              <TableRow className={clsx(props.classes.tableTR)} key={row.seq} id={row.uuid}>
                {options.seq&&(<TableCell className={clsx(props.classes.tableTD)} align={'right'}>{row.seq}</TableCell>)}
                {options.time&&(<TableCell className={clsx(props.classes.tableTD)} align={'center'}>{props.convertTimestamp(row.timestamp)}</TableCell>)}
                {options.ip&&(<TableCell className={clsx(props.classes.tableTD,props.classes.safeTextColor)} align={'right'}><span dangerouslySetInnerHTML={searchString(row.ipaddress,'ipaddress')} /></TableCell>)}
...
//=========================================================================
export default withStyles(styles,{withTheme:true})(yonginTable);